...
jQuery('document').ready(function() {
	...
	var fill_out_form  = jQuery('#fill-out-form');
	var fill_out_radio = jQuery('#fill-out-radio');
	...
	jQuery('.attach_fill_out').change(function() {
	    if (jQuery(this).val() == 'attach') {
            ...
            jQuery('#board-list tbody tr.row td div input.required').each(function(index) {
                if (jQuery(this).val() == '') {
                    remove_row(jQuery(this).parents('tbody'));
                }
            });
            
            jQuery('#stock-list tbody tr.row td div input.required').each(function(index) {
                if (jQuery(this).val() == '') {
                    remove_row(jQuery(this).parents('tbody'));
                }
            });
            ...
            jQuery('#file-upload-validate').attr("disabled", false);
	    }
	    ...
	    if (jQuery(this).val() == 'fill out') {
            ...
            jQuery('#file-upload-validate').attr("disabled", true);
	    }
    });
    ...
    jQuery('#file_upload').uploadify({
		...
		'onComplete' : function(event, ID, fileObj, response, data) {
				...
				jQuery('#file-upload-validate').val(1);
		    }
	});
	...
	jQuery('.remove_file').live('click', function(e) {
		...
		jQuery.ajax({
         ...
         total_files = jQuery('#file-list > li').size() - 1;
 		 if (total_files < 1) {
 		     jQuery('#file-upload-validate').val('');
 		 }
	});
	...
	jQuery('#add-stock').live('click', function(e) {
	    ...
	    if (!jQuery('.stockholder_type').is(":checked")) {
